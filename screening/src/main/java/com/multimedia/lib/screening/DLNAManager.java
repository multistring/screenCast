...
import com.multimedia.lib.screening.log.AndroidLoggingHandler;
...
public final class DLNAManager {
    ...
    public boolean init(@NonNull Context context, @Nullable DLNAStateCallback stateCallback) {
        if (null != mContext) {
...
//            NginxHelper.startNginxServer();
            ...
            new Thread(() -> {
                ...
                final String localInnerPath = mContext.getFilesDir().getAbsolutePath();
                String[] args = {
                        "--host",
...
//        logD("tryTransformLocalMediaAddressToLocalHttpServerAddress ,sourceUrl1  : " + sourceUrl);
        if (!isLocalMediaAddress(sourceUrl)) {
            return sourceUrl;
        }

        boolean isExternal = sourceUrl.startsWith(Environment.getExternalStorageDirectory().getAbsolutePath());
        boolean isInner = sourceUrl.startsWith(context.getFilesDir().getAbsolutePath());

        String newSourceUrl =  null;
        if (isExternal == true) {
            newSourceUrl = getLocalHttpServerAddress(context) +
                    sourceUrl.replace(Environment.getExternalStorageDirectory().getAbsolutePath(), "");
        } else if (isInner == true) {
            newSourceUrl = getLocalHttpServerAddress(context)   //+sourceUrl;
                    +sourceUrl.replace(context.getFilesDir().getAbsolutePath(), "");
        } else {
            return sourceUrl;
        }
        try {
